...
export const ResetPassword = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!token) {
      setStatus('error');
      setMessage('El enlace para restablecer la contraseña es inválido o está incompleto.');
      return;
    }
    ...
    try {
      await apiService.resetPassword(token, contrasena);
      setStatus('success');
      setMessage('Tu contraseña ha sido restablecida exitosamente. Ya puedes iniciar sesión con tu nueva contraseña.');
    } catch (err: any) {
      setStatus('error');
      ...
      >
        ...
        {status === 'success' ? (
          <div className="flex flex-col items-center">
            <div className="w-full p-4 bg-green-50 text-green-700 rounded-xl border border-green-100 text-sm mb-6 text-center">
              {message}
            </div>
            <Link
              to="/"
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={status === 'loading' || !contrasena || !confirmar}
